...
router.post(
  ...
  userController.getAllBackgroundImage
);
router.post(
  "/submit_code_certificate",
  check.role_user,
...
router.post(
  "/get_user_image_toCanvas",
  check.queryFromcanvas_role,
  userController.getUserImageToCanvas
);
router.post(
  "/delete_user_image",
  check.queryFromcanvas_role,
...
router.post(
  "/user_upload_img",
  check.saveCanvas_role,
  userController.checkUploadedImages,
  ...
  function (req, res, next) {
    ...
    form.parse(req, function (error, fields, file) {
      ...
      let b =
        random.getString() +
        uuidv4() +
        path.extname(file.input_img.originalFilename);
      newpath += b;
